...
given rucksack always has the same number of items in each of its two compartments, so
the first half of the characters represent items in the first compartment, while the
...
efforts: here, they are 18 (r) for the first group and 52 (Z) for the second group. The
sum of these is 70.
...
class Year2022Day3Solution(BaseDailySolution):
    YEAR = 2022
    DAY = 3
    ...
    def solve_part_1(cls, input_data: Any) -> int:
        ...
        return sum([process_line(d) for d in input_data])
