...
class Year2021Day7Solution(BaseDailySolution):
    YEAR = 2021
    DAY = 7
    ...
    def solve_part_1(cls, input_data: Any) -> int:
        target = median(input_data)
        return sum([abs(v - target) for v in input_data])

    @classmethod
    def solve_part_2(cls, input_data: Any) -> int:
        # Using 'round' gave inconsistent results so I switched to manually checking both mean options
        target1 = math.ceil(mean(input_data))
        target2 = math.floor(mean(input_data))

        return min(
            sum(sum_sequence(abs(target1 - v)) for v in input_data),
            sum(sum_sequence(abs(target2 - v)) for v in input_data),
        )
