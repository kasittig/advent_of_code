...
by 1, including octopuses that are diagonally adjacent. If this causes an octopus to have an energy level greater than
9, it also flashes. This process continues as long as new octopuses keep having their energy level increased beyond 9.
...
class OctopusGrid(BaseStatusGrid):
    ...
    def do_step(self) -> None:
        ...
        # First, increment all octopi by 1
        self.do_for_each(self.increase_energy)

        while self.needs_flash():
            for i in range(self.max_height):
                for j in range(self.max_width):
                    if self.data_grid[i][j] > 9:
                        self.flash(i, j)
        self.do_for_each(self.reset_energy)
        self.reset_status()
    ...
    def flash(self, i: int, j: int) -> None:
        if self.status_grid[i][j]:
            ...
            self.status_grid[i][j] = True
            # All surrounding octopi have their energy levels incresed
            for ii in [i - 1, i, i + 1]:
                for jj in [j - 1, j, j + 1]:
                    self.increase_energy(ii, jj)

    def increase_energy(self, i: int, j: int) -> None:
        if self.is_out_of_bounds(i, j):
            return
        self.data_grid[i][j] += 1

    def reset_energy(self, i: int, j: int) -> None:
        if self.is_out_of_bounds(i, j):
            return
        if self.status_grid[i][j]:
            self.data_grid[i][j] = 0

    def needs_flash(self) -> bool:
        for i in range(self.max_height):
            for j in range(self.max_width):
                if self.data_grid[i][j] > 9 and not self.status_grid[i][j]:
                    return True
        return False


class Year2021Day11Solution(BaseDailySolution):
    YEAR = 2021
    DAY = 11
    ...
    def solve_part_1(cls, input_data: List[List[int]]) -> int:
        ...
        while steps < 100:
            octopus_grid.do_step()
            steps = octopus_grid.total_steps
    ...
    def solve_part_2(cls, input_data: List[List[int]]) -> int:
        ...
        done = _done(octopus_grid)

        while not done:
            octopus_grid.do_step()
            done = _done(octopus_grid)

        return octopus_grid.total_steps
