...
you consider checking the in-flight menu for one of those drinks that come with a
...
single-instruction infinite loop, never leaving that instruction. If you change almost
any of the jmp instructions, the program will still eventually find another jmp
...
class ToyHandheldGameConsole(object):
    ...
    def step(self, instruction: str) -> int:
        code, value = ToyHandheldGameConsole.parse_instruction(instruction)
        if code == "nop":
            self.nops.append(self.idx)
            self.idx += 1
        elif code == "acc":
            self.idx += 1
            self.acc += value
        else:
            self.jumps.append(self.idx)
            self.idx += value
        return self.idx
    ...
    def run_program(self, instruction_list: list[str]) -> bool:
        visited = [False] * len(instruction_list)

        while not visited[self.idx]:
            visited[self.idx] = True
            self.step(instruction_list[self.idx])
            if self.idx >= len(instruction_list):
                return True
...
class Year2020Day8Solution(BaseDailySolution):
    YEAR = 2020
    DAY = 8
