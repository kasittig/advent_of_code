...
"""
With your neighbor happily enjoying their video game, you turn your attention to an
open data port on the little screen in the seat in front of you.
...
receive should be the sum of any two of the 25 immediately previous numbers. The two
numbers will have different values, and there might be more than one such pair.

For example, suppose your preamble consists of the numbers 1 through 25 in a random
order. To be valid, the next number must be the sum of two of those numbers:
...
than 25 numbers ago) was 20. Now, for the next number to be valid, there needs to be
...
found: you must find a contiguous set of at least two numbers in your list which sum to
...
number from step 1, 127. (Of course, the contiguous set of numbers in your actual list
...
def is_window_valid(window: list[int]) -> bool:
    target = window[-1]
...
def check_encoding(data: list[int], window_size: int) -> Optional[int]:
...
def find_window_sums_to_target(
    input_data: list[int], target: int
) -> Tuple[Optional[int], Optional[int]]:
    for i in range(len(input_data)):
        for j in range(len(input_data)):
            if sum(input_data[i:j]) == target:
                return min(input_data[i:j]), max(input_data[i:j])
    return None, None


class Year2020Day9Solution(BaseDailySolution):
    YEAR = 2020
    DAY = 9
    ...
    def solve_part_1(cls, input_data: list[int]) -> int:
        return check_encoding(input_data, 25)

    @classmethod
    def solve_part_2(cls, input_data: Any) -> Any:
        target = check_encoding(input_data, 25)
        return sum(find_window_sums_to_target(input_data, target))
