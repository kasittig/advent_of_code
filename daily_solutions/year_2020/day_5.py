...
proceeds again, this time with only three steps. L means to keep the lower half, while
...
your list. However, there's a catch: some of the seats at the very front and back of
...
def parse_seat_id(seat_str: str) -> int:
    seat_binary_str = (
        seat_str.replace("F", "0").replace("B", "1").replace("R", "1").replace("L", "0")
    )
    row_binary_str = seat_binary_str[:7]
    col_binary_str = seat_binary_str[7:]

    return int(row_binary_str, 2) * 8 + int(col_binary_str, 2)


class Year2020Day5Solution(BaseDailySolution):
    YEAR = 2020
    DAY = 5

    @classmethod
    def format_data(cls, input_data: list[str]) -> list[int]:
        return list(map(parse_seat_id, input_data))
